...
export class ScriptLetContext {
    ...
    addDeclaration(declaration, parent, ...callbacks) {
        const range = getNodeRange(declaration, this.ctx.code);
        const part = this.ctx.code.slice(...range);
        this.appendScript(`${part};`, range[0], this.currentScriptScopeKind, "VariableDeclaration", (st, tokens, _comments, result) => {
            const node = st;
            for (const callback of callbacks) {
                callback(node, result);
            }
            ...
            result.addPostProcess(() => {
                fixLocations(node, [], [], range[0] - node.range[0], result.visitorKeys, this.ctx);
