<script lang="ts">
	...
	let myFoldersContentHeight = $state(0);
	let isMyFoldersExpanded = $state(false);
...
<main>
	...
	<section class="mb-10 animate-in fade-in slide-in-from-bottom-8 duration-500 min-h-20">
		...
		<div
			class="relative overflow-hidden transition-[max-height] ease-in-out {!isMyFoldersExpanded &&
			isMyFoldersOverLimit
				? 'max-h-[1000px]'
				: 'max-h-none'}"
		>
			<div bind:clientHeight={myFoldersContentHeight}>
											...
											<div class="group">
												{#if liveTag}
													<div class="flex items-center">
														...
														<!-- Add to bookmark -->
														<div
															class="ml-1 size-5 items-center flex opacity-30 group-hover:opacity-100 group-hover:text-muted-foreground"
														>
			...
			<!-- gradient overflow -->
			{#if !isMyFoldersExpanded && isMyFoldersOverLimit}
				<div
					class="absolute bottom-0 left-0 right-0 h-32 bg-gradient-to-t from-background via-background/60 to-transparent pointer-events-none"
				></div>
			{/if}
		</div>
		<!-- overflow folders -->
		{#if isMyFoldersOverLimit}
			<div class="flex justify-center mt-4">
				<Button
					...
					onclick={(e) => {
						isMyFoldersExpanded = !isMyFoldersExpanded;
				...
				>
					{#if isMyFoldersExpanded}
						<ArrowUpFromLine /> Show less
					{:else}
						<ArrowDownFromLine /> Show more
					{/if}
				</Button>
			</div>
		{/if}
	...
	>
		...
		<div
			class="relative overflow-hidden transition-[max-height] ease-in-out {!isPublicFoldersExpanded &&
			isPublicFoldersOverLimit
				? 'max-h-[1000px]'
				: 'max-h-none'}"
		>
			<div bind:clientHeight={publicFoldersContentHeight}>
			...
			<!-- gradient overflow -->
			{#if !isPublicFoldersExpanded && isPublicFoldersOverLimit}
				<div
					class="absolute bottom-0 left-0 right-0 h-32 bg-gradient-to-t from-background via-background/60 to-transparent pointer-events-none"
				></div>
			{/if}
		...
		{#if isPublicFoldersOverLimit}
			<div class="flex justify-center mt-4">
				<Button
					...
					class="gap-1.5 shadow-xs"
					onclick={(e) => {
						isPublicFoldersExpanded = !isPublicFoldersExpanded;
						if (!isPublicFoldersExpanded) {
							const target = e.currentTarget.closest('section');
							target?.scrollIntoView({ behavior: 'smooth', block: 'start' });
						}
					}}
